...
- **Фаза 0** — Квалификация GDPR и AI Act: Block L определяет `gdpr_mandatory`, затем Block M определяет `ai_type` и `ai_act_status`. Если `M2 = No`, профильные вопросы не задаются; при `gdpr_mandatory = True` бот запускает чек-лист применимых GDPR-мер, а при `gdpr_mandatory = False` завершает аудит без чек-листа.
...
- **Фаза 2** — Чек-лист мер: персонализированный список требований с подсказками из Google Sheets. `ai_type = "model"` включает только модельные/GPAI требования и игнорирует продуктовые риски; `ai_type = "system"` включает продуктовые риски и role shift по ст. 25 AI Act.
- **Фаза 3** — Отчет: итоговый документ с рекомендациями и юридическим комментарием. Для `AIA_OUT_OF_SCOPE` отчет показывает GDPR-задачи без AI Act регуляторных шагов; для `PROHIBITED_RISK` стандартные задачи заменяются юридическим блоком по ст. 5 AI Act; для high-risk/GPAI добавляются регуляторные следующие шаги.
...
| M1 | Вопрос AI Act... | | M2; `ai_type = "model"` | M2; `ai_type = "system"` |

В этой же вкладке после `L1-L4` должны лежать строки `M1` и `M2`: бот читает из них тексты вопросов AI Act Screening. На `M1` бот показывает кастомные кнопки `📦 Независимая модель` и `🖥 Готовая система`, а не Yes/No. Старые значения `EXIT_ANON`, `EXIT_GDPR`, `WARN_ATTACK` могут оставаться в таблице как человеческие пометки, но код не использует их для маршрутизации или установки флагов.
